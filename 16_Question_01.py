# =====================================================================
# 종합 실습 1. 설비 종합 모니터링 리포트
# 요구사항 자세히는 practice_guide.md 참고
# =====================================================================

sensors = [
    ("컨베이어_01", 78, 2.1),
    ("용접기_02", 92, 5.4),
    ("절단기_03", 85, 3.2),
    ("건조로_04", 101, 6.8),
    ("냉각탑_05", 67, 1.5),
    ("도장부스_06", 88, 4.1),
    ("성형기_07", 90, 2.9),
]
# (설비명, 온도, 진동)

# 판정 기준
#   온도 > 90 또는 진동 > 5.0  > "위험"
#   온도 >= 80 또는 진동 >= 3.0 > "주의"
#   그 외                      > "정상"

status_list = []
normal = warning = danger = 0

# TODO 1. 각 설비 상태 판정해서 번호 붙여 한 줄씩 출력 (for + enumerate + if/elif/else)

num = 1
for name, temp, vib in sensors:
    if temp > 90 or vib > 5.0:
        status = "위험"
        danger += 1
    elif temp >= 80 or vib >= 3.0:
        status = "주의"
        warning += 1
    else:
        status = "정상"
        normal += 1

    status_list.append((name, temp, vib, status))
    print(f"{num}, {name} -> {status}")
    num += 1

# TODO 2. 정상 / 주의 / 위험 각각 몇 대인지 세서 출력 (누적변수)

normal = warning = danger = 0
for _, _, _, s in status_list:
    if s == "정상":
        normal += 1
    elif s == "주의":
        warning += 1
    else:
        danger += 1

print(f"\n정상: {normal}대 / 주의: {warning}대 / 위험: {danger}대")

# TODO 3. 이상 설비(주의 + 위험) 비율 % 출력 (round)

ratio = round((warning + danger) / len(sensors) * 100, 1)
print(f"이상 비율: {ratio}%")

# TODO 4. 전체 평균 온도 출력 (round)

avg_temp = round(sum(t for _, t, _ in sensors) / len(sensors), 1)
print(f"평균 온도: {avg_temp}°C")

# TODO 5. 온도 가장 높은 설비 이름 + 온도 출력 (반복문으로 직접 찾기)

max_temp = 0
max_name = ""

for name, t, _ in sensors:
    if t > max_temp:
        max_temp = t
        max_name = name

print(max_name, max_temp, "°C")

# TODO 6. "위험" 설비 이름만 모아서 정렬해 리스트로 출력 (.append() + .sort())

danger_list = []
for name, _, _, status in status_list:
    if status == "위험":
        danger_list.append(name)

danger_list.sort()
print(f"위험 설비 : {danger_list}")

# 도전) 위험 1대라도 있으면 "⚠ 즉시 점검 요망", 없으면 "✅ 전 설비 안정"
